...
msg_distinto_fn: .asciiz "\nNumero de Filas distinto a numero de Columnas \n"
cambio: .asciiz "\nIndice de la fila/columna a intercambiar: "
msg_mal_indice: .asciiz "\nDimensión incorrecta\n"
msg_fin:    .asciiz "\nFin del programa.\n"
...
main:
	...
	beq $t4,5,opcion5 # Si se inserta un 5 intercambiar fila por la columna
    fin_opciones:
	...
	# opción 5(intercambiar filas x columna)
	opcion5:
		la $s3,mat # direccion base de la matriz

		lw $s1,nfil # Cargo las filas en s1
		lw $s2,ncol # cargo las columnas en s2
		bne $s1,$s2,error_dim # Si no coinciden las filas y columnas mostrar mensaje de error_columna
		# Pedir índice de fila y columna
		li $v0,4
		la $a0,cambio # Pide la fila
		syscall
		li $v0,5
		syscall
		move $t1,$v0 # Muevo la fila a t1
		bltz $t1,error_indice
		beq $t1,$s1,error_indice

		move $t3,$zero # recorrer filas
		move $t4,$zero # recorrer columnas

		intercambio:
		# Elementos de la fila

		mul $t5,$t1,$s2 # f*ncol
        	add $t5,$t5,$t4 # f*ncol+c
        	mul $t5,$t5,size
        	add $t5,$t5,$s3 # dirección [f][c]
		lw $t6,($t5) # cargo el elemento en t6

		# Elementos de la columna
		mul $t7,$t3,$s2 # f*ncol
        	add $t7,$t7,$t1 # f*ncol+c
        	mul $t7,$t7,size
        	add $t7,$t7,$s3 # dirección [f][c]
		lw $t8,($t7) # cargo el elemento en t6
	...
	error_dim:
	li $v0,4
	la $a0,msg_distinto_fn
	syscall
	b mostrar_matriz
	error_indice:
	li $v0,4
	la $a0,msg_mal_indice
	syscall
	b mostrar_matriz
    error_opcion:
    li $v0,4
